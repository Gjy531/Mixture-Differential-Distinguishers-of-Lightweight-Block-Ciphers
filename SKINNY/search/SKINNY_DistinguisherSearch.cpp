...
void parse_sol_Skinny(GRBModel& model, int r, int printMode){
    ...
    for (int j = 1; j <= r; j++){
        for (int i = 0; i < 3; i++){
            ...
            for (int c = 0; c < 4; c++){//The first line variable is newly defined for each round
                c_map[i][c] = model.getVarByName("c_"+to_string(i)+"_"+to_string(j)+"_"+to_string(c));
            }
            for (int c = 4; c < 8; c++){//The second line variable is obtained from the first line (permutation) of the previous round
                c_map[i][c] = model.getVarByName("c_"+to_string(i)+"_"+to_string(j-1)+"_"+to_string(Skinny_perm[c-4]));
            }
            for (int c = 8; c < 16; c++){//The third and fourth lines of variables are newly defined for each round
                c_map[i][c] = model.getVarByName("c_"+to_string(i)+"_"+to_string(j)+"_"+to_string(c));
            }
...
GRBModel genModel(GRBEnv env, int r){
    ...
    for (int i = 1; i <= r; i++){
        ...
        for (int k = 0; k< 4; k++){ 
            //The second line variable is directly obtained from the first line of the previous round (permutation)
            //Add variables to the remaining three lines
            cr_2[k+4] = c_map[2][Skinny_perm[k]];
            cr_1[k+4] = c_map[1][Skinny_perm[k]];
            cr_0[k+4] = c_map[0][Skinny_perm[k]];

            //Add a new variable on the first line
        ...
        for (int k = 0; k< 8; k++){ 
            //Add new variables in the third and fourth lines
