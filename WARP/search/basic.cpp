...
string input_pattern(GRBModel& model){
    vector<string> Ind = {"01", "02", "03", "12", "13", "23"};
    unordered_map<string, vector<GRBVar>> e_map;
    unordered_map<string, string> e_value;
    for (int i = 0; i < 6; i++){
    ...
    return value_to_pattern(e_value);
}
void parse_inputpatterns(vector<string> inputpatterns){
    vector<string> temp;
    for (int i = 0; i < inputpatterns.size(); i++){
        temp.push_back(inputpatterns[i]);
    }
    sort(temp.begin(), temp.end());
    temp.erase(unique(temp.begin(), temp.end()), temp.end());
    cout<< "There are "<< temp.size()<<" input patters."<<endl;
    if (temp.size() == inputpatterns.size()){
        cout << "No hull effect"<<endl;
        return;
    }
    vector<int> count_unique;
    for (int i = 0; i < temp.size(); i++){
        count_unique.push_back(count(inputpatterns.begin(), inputpatterns.end(), temp[i]));
    }
    vector<int> index;
    for (int i = 0; i < temp.size(); i++){
        index.push_back(i);
    ...
    sort(index.begin(), index.end(), [&](const int & a, const int& b) {return (count_unique[a] > count_unique[b]);});

    for (int i = 0; i < index.size(); i++){
        cout << count_unique[index[i]]<<", ";
    }
    cout << endl;
}
void parse_MC_sol(GRBModel& model){
    vector<string> Ind = {"01", "02", "03", "12", "13", "23"};
    unordered_map<string, vector<GRBVar>> e_map, er_map;
    unordered_map<string, string> e_value, er_value;
    for (int i = 0; i < 6; i++){
        e_map[Ind[i]] = getVarVectorByName(model, "e"+Ind[i]+"_0_", 4);
        e_value[Ind[i]] = getVectorVarValue(model, e_map[Ind[i]]);

        er_map[Ind[i]] = getVarVectorByName(model, "e"+Ind[i]+"_1_", 4);
        er_value[Ind[i]] = getVectorVarValue(model, er_map[Ind[i]]);
    }
    cout<<value_to_pattern(e_value)<<value_to_pattern(er_value)<<",";
}
void parse_sol(GRBModel& model, int r, int printMode){
    ...
    }else if (printMode == 0) {// print the input pattern only
        cout<<"\'"<<value_to_pattern(e_value)<<"\', "; 
    }else {
