...
UINT64 * quadruple(uint8_t rd, uint8_t rd_for_rand, uint8_t tk[][16], uint8_t tk_for_rand[][16], UINT64 N3, string inpatt, string outpatt){
    ...
    */
    ...
    while (bnum < N3){
        ...
        //Generate quads that meet input patterns
...
UINT64 * testOnMultKey(uint8_t rd, uint8_t rd_for_rand, UINT64 N2, UINT64 N3, string inpatt, string outpatt){
    ...
    */
    ...
    uint8_t tk[rd][16];
    uint8_t tk_for_rand[rd_for_rand][16]; //A round key variable specifically designed for high round encryption
    UINT64 bnum=0;
    static UINT64 count[2];
    UINT64 *count_itr;
    count[0] = 0;
    ...
    while (bnum < N2){
        ...
        initialize((rd_for_rand - 2)/2, key0, key1, master_tweak, tk_for_rand); //Generate a round key variable specifically for high round encryption
...
int main(int argc, char** argv){
    ...
    int rd_for_rand = atoi(argv[2]); //The second parameter entered on the command line is used to set the number of encryption rounds for simulating random permutation
    ...
    */
    ...
    proc_time = MPI_Wtime();
    count = testOnMultKey(rd,rd_for_rand, N2,N3,inpatt,outpatt); //Add a parameter to simulate the number of encryption rounds for random permutation
